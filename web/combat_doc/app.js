...
function buildJsonPayload(meta, attacks) {
  return {
    meta,
...
async function documentFromClient(form) {
  ...
  const attacks = loadRows(parsed.data, "", null);
  ...
  const md = renderMarkdown(meta, attacks, []);
  const json = buildJsonPayload(meta, attacks);
  applyResult({
    ok: true,
